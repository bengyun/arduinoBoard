...
#define SECRET_GPRS_APN         "xz01.njm2mapn"    // replace your GPRS APN
...
GSM           gGSMAccess;
GPRS          gGPRS;
GSMClient     gGSMClient;                          // Used for the TCP socket connection
GSMSSLClient  gGSMSSLClient;
#if SSL_CONNECT
#define       WEB_CLIENT        gGSMSSLClient      //
MqttClient    gMQTTClient(gGSMSSLClient);          // 通过SSL/TLS连接创建MQTT客户端
#else
#define       WEB_CLIENT        gGSMClient         //
MqttClient    gMQTTClient(gGSMClient);             // 通过TCP连接创建MQTT客户端
#endif
#if WATCH_DOG
